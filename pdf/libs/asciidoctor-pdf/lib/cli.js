...
async function convertFiles (files, argv, options, verbose, preview, onlyHTML) {
  for (const file of files) {
    if (verbose) {
      console.log(`converting file ${file.contents ? '-' : file.path}`)
    }
    await converter.convert(processor, file, options, argv.timings, argv.watch, preview, onlyHTML)
  }
}
...
class PdfOptions {
  constructor () {
    ...
    this.options = new Options(defaultOptions)
      ...
      .addOption('only-html', {
        default: false,
        describe: 'only generated HTML document without PDF',
        type: 'boolean'
      })
  }
...
class PdfInvoker extends Invoker {
  async invoke () {
    ...
    const { verbose, version, files, watch, 'template-require': templateRequireLib, preview, 'only-html': onlyHTML } = args
    ...
    if (cliOptions.stdin) {
      ...
      await convertFiles(fileObjects, args, asciidoctorOptions, verbose, preview, onlyHTML)
      return { exit: false }
