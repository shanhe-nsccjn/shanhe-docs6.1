...
async function convert (processor, inputFile, options, timings, watch, preview, onlyHTML) {
  ...
  try {
    if (fs.existsSync(tempFile)) {
      ...
      // 移除 表格内部的说明标题的序号
      $('div.admon [id]').each(function() {
        $(this).html(
          $(this).text().replace(/^\d\. +/, '')
        );
      });
      fs.writeFileSync(tempFile, $.html().replace('Array.from(previousRow.cells)', 'Array.from(previousRow.cells || [])'));
    }
    if (!onlyHTML) {
