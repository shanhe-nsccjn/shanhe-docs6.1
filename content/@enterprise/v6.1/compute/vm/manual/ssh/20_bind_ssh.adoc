...
. 输入 SSH 密钥的名称，选择**创建新密钥对**，并选择加密方法。
...
. 点击**是**，弹出**选择云服务器**窗口。
+
image::/images/cloud_service/compute/vm/vm_ssh_load_vm.png[,60%]

. 选择云服务器，并确定是否勾选**加载 SSH 密钥后，禁止密码登录，同时 WebSSH 连接密码认证不可用**。
. 点击**提交**，完成 SSH 密钥绑定到云服务器的操作。
+
...
. 进入云服务器列表页面。
. 在左侧导航栏中，点击 *SSH 密钥*，进入 **SSH 密钥**页面。
. 勾选已创建的 SSH 密钥，点击**加载到云服务器**，弹出**选择云服务器**窗口。
+
image::/images/cloud_service/compute/vm/vm_ssh_bind.png[]


. 选择创建的云服务器，点击**提交**，完成 SSH 密钥绑定操作。
+
根据需要确定是否勾选**加载 SSH 密钥后，禁止密码登录，同时 WebSSH 连接密码认证不可用**。
+
image::/images/cloud_service/compute/vm/vm_ssh_bind_win.png[,60%]
...
include::../../../../../../_components/table_footer.adoc[]
+
image::/images/cloud_service/compute/vm/vm_ssh_create_vm.png[,60%]

. 云服务器创建完成后，您可以在云服务器详情页面，查看已绑定的 SSH 密钥。
+
image::/images/cloud_service/compute/vm/vm_ssh_bind_site.png[]
