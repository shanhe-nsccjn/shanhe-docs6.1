...
keywords: 云计算,大数据,消息队列,中间件,RocketMQ,Topic,client
weight: 20
draft: false
---
本小节主要介绍如何通过 RocketMQ 客户端命令行对 Topic 进行管理。

下面列举几个常见操作，详情可参见link:http://rocketmq.apache.org/docs/cli-admin-tool/[官方使用文档]。
...
* `-c` 参数后面指定集群名称。（集群名称可在link:../../config_para/modify_para[配置参数]页面进行设置。默认为 `DefaultCluster`。）
