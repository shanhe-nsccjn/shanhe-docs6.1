...
weight: 6
collapsible: false
keywords: 秘钥签名, ShanHe, 山河云, 签名方法, API
---
这里介绍 API 请求中签名 ( signature ) 的生成方法。您可以根据实际情况选择使用 *API 密钥* 或 link:../../../authorization/iam/[IAM] *身份* 来完成签名。

== API 密钥签名

您需要先在控制台创建 link:https://console.shanhe.com/access_keys/[API 密钥]，获得 accesss_key_id 和 secret_access_key，这里我们假设
